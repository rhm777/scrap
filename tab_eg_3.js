...
// amazon.com.
// selector for rgns: $x("//div[contains(@class,'yuRUbf')]/a")[2].click()

var get_click_regions = async function ( page , selector ,search_word  )
{
    if ( search_word == undefined )
        search_word = "air cooler"
    page.wait  (3000 );
    let  sel_fill = "//input[@name='field-keywords']";
    await  page.page.waitForXPath ( sel_fill );
    let search_box = await  page.page.$x  ( sel_fill )
    await search_box[0].type ( search_word );
    console.log ( " ************** ", await search_box[0].getProperty("value") );
    let  sel_sub   = "//input[@type='submit']"
    await  page.page.waitForXPath ( sel_sub );
    let submit_button = await  page.page.$x ( sel_sub );
   // console.log ( submit_button.length);
    // await submit_button[0].hover();
    await submit_button[0].click();
    await page.wait  (10000 );
    let sel_links = "//h2/a"
    await  page.page.waitForXPath ( sel_links );
    let clck_rgns =  await page.page.$x ( sel_links );
    return clck_rgns;  
}
...
var run = async function ()
{
   ...
   let pg = await brws.load_new_page ( "https://www.amazon.com" , 1000 );
   ...
   //let  sel  = "//a";
   var rgns =  await get_click_regions ( pg , "" , "air cooler"  );
   
   console.log ( "rgns length: " , await rgns.length );
   let i = 0;
    for ( i = 0 ; i < rgns.length; i++ )
    {
      console.log ( await (await rgns[i].getProperty("textContent")).jsonValue()  )
      console.log ( "" );
      console.log ( "" );
    }
  // await brws.load_clicks_one_by_one ( pg , rgns.slice(0,3) , 4000 );
  // await brws.activate_pages_one_by_one ( 10000 );
   await brws.load_clicks_one_by_one_w_name  ( "r", pg , rgns.slice(0,13) , 10000 );
   await brws.activate_clicks_one_by_one_w_name ( "r" , 7000 );
};
