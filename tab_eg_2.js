...
var get_click_regions = async function ( page , selector ,search_word  )
{
    if ( search_word == undefined )
        search_word = "puppeteer"
    page.wait  (3000 );

    await  page.page.waitForXPath ( "//input[@name='q']" );
    let search_box = await  page.page.$x ("//input[@name='q']")
    await search_box[0].type ( search_word );
    //await page.wait  (1000 );
    await  page.page.waitForXPath ( "//input[@name='btnK']" );
    let submit_button = await  page.page.$x ("//input[@name='btnK']")
    console.log ( submit_button.length);
    // await submit_button[0].hover();
    await submit_button[1].click();
    //await page.wait  (4000 );
    await  page.page.waitForXPath ( "//h3[contains(@class,'LC20lb')]" );
    let clck_rgns =  await page.page.$x ( "//h3[contains(@class,'LC20lb')]" );
    return clck_rgns;  
}
...
var run = async function ()
{
   ...
   let pg = await brws.load_new_page ( "https://www.google.com" , 1000 );
   ...
   //let  sel  = "//a";
   var rgns =  await get_click_regions ( pg , "" , "puppeteer xpath examples"  );
